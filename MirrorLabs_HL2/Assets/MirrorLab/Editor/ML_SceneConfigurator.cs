...
namespace MirrorLabs
{
    public class MLSceneConfigurator : EditorWindow
    {
        ...
        private string scenePath = "Assets/MirrorLab/Scenes/";
        ...
        */
        ...
        private void OnGUI()
        {
            ...
            // MRTK setup (Work in progress)
            /*
            includeMRTK = EditorGUILayout.BeginToggleGroup(new GUIContent("Inlcude MRTK", "Add and configure the Mixed Reality Toolkit based on parameters below. The MRTK foundation needs to be present in the asset folder"), includeMRTK);
            GUILayout.Label("Please define the desired MRTK configuration.");
            isHoloLens1 = EditorGUILayout.Toggle(new GUIContent("     HoloLens V1", "Configure the MRTK for HoloLens 1."), isHoloLens1);
            isHoloLens2 = EditorGUILayout.Toggle(new GUIContent("     HoloLens V2", "Configure the MRTK for HoloLens 2."), isHoloLens2);
        ...
        void GenerateScene(string name, GameObject robot, string robotName)
        {
            ...
            if (includeMRTK)
            {
                ConfigureMRTK(isHoloLens1, isHoloLens2);
            }
            */
        ...
        /*  MRTK integration (work in progress)
        private void ConfigureMRTK(bool HL1, bool HL2)
        {
            if (!Directory.Exists("Assets/MRTK"))
            {
                Debug.Log("The Mixed Reality Toolkit was not found in your assets. Please add the MRTK and restart the Scene Configurator.");
            ...
            if ( HL1 != HL2)
            {
                hlv = (HL1 == true) ? "HL1" : "HL2";
            }
            else
            {
                Debug.Log("MRTK profile configured for HoloLens 2 by default, please switch the profile according to your intended device");
            }
            
            GameObject mrtk = new GameObject("MixedRealityToolkit");
            mrtk.AddComponent<MixedRealityToolkit>();
            
            var profile = AssetDatabase.LoadAssetAtPath<MixedRealityToolkitConfigurationProfile>(prefabPath +"MRTK_Profiles/MirrorLabs_" + hlv+ "_MRTK_Profile.asset");
            MixedRealityToolkit mrtkProfile = mrtk.GetComponent<MixedRealityToolkit>();
            mrtkProfile.ActiveProfile = profile;
            Debug.Log("MRTK Profil set sucessful, please ignore the initilization errors");
        }
        */
