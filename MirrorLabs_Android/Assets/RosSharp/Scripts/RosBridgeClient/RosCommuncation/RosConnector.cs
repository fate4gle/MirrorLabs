...
namespace RosSharp.RosBridgeClient
{
    public class RosConnector : MonoBehaviour
    {
        ...
        public string RosBridgeServerUrl = "ws://192.168.0.103:9090";
        ...
        private bool[] isActiveBehavior;


            
         public void InitRosConnector(string IP, string Port)
         {
            // Log all active subscribers/publishers, deactivate them and close existing RosSocket
            try
            {
                activeBehavior = GetComponents<Behaviour>();
                isActiveBehavior = new bool[activeBehavior.Length];
                
                for (int i = 0; i< activeBehavior.Length; i++)
                /*{
                    Debug.Log(activeBehavior);
                    isActiveBehavior[i] = activeBehavior[i].isActiveAndEnabled;
                    activeBehavior[i].enabled = false;
                }
                */
        ...
        private void RosConnectionSuccess()
        {
            ...
            this.GetComponent<JointStateSubscriber>().enabled = true;
            //Reactive all previously active subscribers/publishers
            /*
            for (int i = 0; i < activeBehavior.Length; i++)
            {
                activeBehavior[i].enabled = isActiveBehavior[i];
                //Debug.Log("enabling " + activeBehavior[i].name.ToString());
            }
            */
